...
Sự phân biệt này có những hệ quả khá sâu sắc đối với các phương pháp trong deep RL. Một số họ thuật toán chỉ có thể được áp dụng trực tiếp trong một trường hợp, và sẽ phải được sửa đổi đáng kể để áp dụng cho trường hợp còn lại.

#### 3.3. Policies

Policy là quy luật mà agent dùng nó để quyết định hành động. Nó có thể là cứng và được kí hiệu là $$\mu$$,

$$a_t = \mu(s_t)$$

hoặc được lấy mẫu từ một distribution và được kí hiệu bằng $$\pi$$:

$$a_t \sim \pi(\cdot|s_t)$$

Bởi vì policy như là não của agent nên sẽ có nhiều blogs và papers sử dụng agent và policy với ý nghĩa như nhau. 
