...
$$\mathbb{P}_r = \int_{\chi} d\gamma(x, y)dx$$

$$\mathbb{P}_g = \int_{\chi} d\gamma(x, y)dy$$

Áp dụng Larange multipliers, ta được hàm tối ưu như sau: 

$$L(\pi, f, g) = \int_{\chi \times \chi}||x-y||_2 \pi(x, y)dxdy + \int_{\chi}(p(x) - \int_{\chi}\pi(x, y)dy)f(x)dx + \int_{\chi}(q(y) - \int_{\chi}\pi(x, y)dx)g(y)dy$$

$$L(\pi, f, g) = \mathbb{E_{x \sim p}}[f(x)] + \mathbb{E_{y \sim q}}[g(y)] + \int_{\chi \times \chi}(||x-y||_2 - f(x) - g(y)) \pi(x, y)dxdy$$
...
$$W(\mathbb{P_r}, \mathbb{P_\theta}) =  \sup_{f, g} \inf_{\pi} \mathbb{E_{x \sim p}}[f(x)] + \mathbb{E_{y \sim q}}[g(y)] + \int_{\chi \times \chi}(||x-y||_2 - f(x) - g(y)) \pi(x, y)dxdy$$

Ở phương trình trên, ta có 2 expections độc lập với $\pi$ nên có thể rút ra ngoài. 

$$W(\mathbb{P_r}, \mathbb{P_\theta}) =  \sup_{f, g} \mathbb{E_{x \sim p}}[f(x)] + \mathbb{E_{y \sim q}}[g(y)] + \inf_{\pi} \int_{\chi \times \chi}(||x-y||_2 - f(x) - g(y)) \pi(x, y)dxdy$$

Đối với vế sau của phương trình, 

$$\inf_{\pi} \int_{\chi \times \chi}(||x-y||_2 - f(x) - g(y)) \pi(x, y)dxdy = 0 \quad \text{if } ||x-y||_2 \ge f(x) + g(y)$$

Dùng điều kiện trên để thay vào (), ta được: 

$$
W(p, p_g) = \sup_{\substack{f, g \\ f(x) + g(y) \leq ||x - y||_2}} L(\pi, f, g) = \sup_{\substack{f, g \\ f(x) + g(y) \leq ||x - y||_2}} \mathbb{E_{x \sim p}}[f(x)] + \mathbb{E_{y \sim q}}[g(y)]
$$
...
Bây giờ chúng ta sẽ biến đổi ràng buộc $f(x) + g(y) \leq ||x - y||_2$ sang một biểu thức đơn giản để tính hơn như sau:
...
$$
\mathbb{E_{x \sim p}}[f(x)] + \mathbb{E_{y \sim q}}[g(y)] \leq \mathbb{E_{x \sim p}}[\kappa(x)] - \mathbb{E_{y \sim q}}[\kappa(y)]
$$
