...
Vložte si do projektu podkladovou mapu. Doporučuji službu Základní mapy ČR, kterou poskytuje Český úřad zeměměřičský a katastrální. Adresu služby přidejte přes Open Data Source Manager (zkratka Ctrl+L, nebo CMD+L na macu) a vyberte možnost ArcGIS Map Services. Na záložce Server Connection klikněte na New a vložte adresu {% highlight %} https://ags.cuzk.cz/ArcGIS/rest/services/zm/MapServer {% endhighlight %} do řádku URL. Připojení pojmenujte, v programu zůstane uloženo. Do mapového okna přidáte tlačítkem Connect a tlačítkem Add v spodní části stránky. 
...
Dále si připojte Ortofotomapu a službu SM5V. Adresa služby ortofoto je {% highlight %} https://geoportal.cuzk.cz/WMS_ORTOFOTO_PUB/WMService.asp {% endhighlight %} adresa mapové služby SM5V – státní mapa 1:5000 vektorová je {% highlight %} https://geoportal.cuzk.cz/WMS_SM5V_PUB/WMService.aspx {% endhighlight %}
