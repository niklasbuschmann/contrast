...
In college I took MATH360 which was Mathematics of Info Security. In it we learned a lot of intro basics about cryptography and encryption. In class we were allowed to use anything we could code, thank god that was my major. Unfortunately I struggled a bit trying to make methods and functions that would help me on the material when I first attempted to code in C++. We were dealing with such large numbers that 64 signed bits was not enough to represent them, and in C++ you must specify unsigned variables and then I had to make them of type Long Long. Having ULL or unsigned long long every where made for very long and unreadable code, so I switched to the less verbose and more intuitive python. This worked 98% of the time. Only a few instances were my calculations or the numbers we were dealing with were still to big for my coding abilities and language experience.
...
Sidenote: Due to number size constraints a few of these were actually done through sage on [cocalc](https://cocalc.com). This will be noted above the code highlighting What environment/language the code was done in. This code was also not assigned/required/or turned in for a grade other than to show MY work on class assignments. So this should not be breaking any rules, especially since the professor would even let us go over code together to do problems. If this changes then the code I wrote will be hidden for now until I graduate. If this happens then it may be possible for you to request to see the code yourself as someone not attending CSU, looking to hire me, or a friend looking to satisfy your curiosity. Thank you for understanding :)
